	...
	if (button & 1) {
		if (mapState[y][x] != TILE_OCCUPIED) {
			if (!preview)
				return;
			// Check if valid.
			if (preview->GetTurretId() == 3 || preview->GetTurretId() == 4) return;
			...
			// Purchase.
			if ((preview->GetTurretId() == 2) && (mark == 1))
			{
				dynamic_cast<OrbTurret*>(preview)->cur = bull;
				mark = 0;
			}
			else if ((preview->GetTurretId() == 2) && (mark == 0))
			{
				dynamic_cast<OrbTurret*>(preview)->cur = 4;
				mark = 0;
			}
			if (markshift == 0) EarnMoney(-preview->GetPrice());
			else markshift = 0;
			// Remove Preview.
		...
		else
		{
			...
			{
				IObject* temp = dynamic_cast<IObject*>(it);
				if ((preview->Position.x == temp->Position.x) && (preview->Position.y = temp->Position.y))
				{
					if ((preview->GetTurretId() == 1) && (dynamic_cast<Turret*>(temp)->GetTurretId() == 1))
					{
						if (dynamic_cast<Turret*>(temp)->level == 2) break;
						dynamic_cast<Turret*>(temp)->level++;
						Engine::Sprite upgrade("play/turret-2.png", temp->Position.x, temp->Position.y);
						std::swap(dynamic_cast<Turret*>(temp)->Sprite::bmp, upgrade.bmp);
						EarnMoney(-preview->GetPrice());
						break;
					}
					if (preview->GetTurretId() == 3)
					{
						EarnMoney(dynamic_cast<Turret*>(temp)->GetPrice() / 2);
						if (dynamic_cast<Turret*>(temp)->level == 2)
						{
							EarnMoney(dynamic_cast<Turret*>(temp)->GetPrice() / 2);
						}
						if (dynamic_cast<Turret*>(temp)->GetTurretId() == 2)
						{
							dynamic_cast<OrbTurret*>(temp)->RemoveBullet();
						}
						TowerGroup->RemoveObject(temp->GetObjectIterator());
						mapState[y][x] = TILE_FLOOR;
					}
					if (preview->GetTurretId() == 4)
					{
						if (dynamic_cast<Turret*>(temp)->GetTurretId() == 0)
						{
							preview = new PlugGunTurret(0, 0);
						}
						else if (dynamic_cast<Turret*>(temp)->GetTurretId() == 1)
						{
							preview = new MachineGunTurret(0, 0);
							if (dynamic_cast<Turret*>(temp)->level == 2)
							{
								dynamic_cast<Turret*>(preview)->level++;
								Engine::Sprite upgrade("play/turret-2.png", temp->Position.x, temp->Position.y);
								std::swap(dynamic_cast<Turret*>(preview)->Sprite::bmp, upgrade.bmp);
							}
						}
						else if (dynamic_cast<Turret*>(temp)->GetTurretId() == 2)
						{
							preview = new OrbTurret(0, 0);
							Engine::LOG(Engine::INFO) << 1;
							bull = dynamic_cast<OrbTurret*>(temp)->bulletCount;
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
		...
		std::vector<int>::const_iterator vct = code.begin();
		int flag = 1;
		for (auto lst = keyStrokes.begin(); lst != keyStrokes.end(); lst++, vct++)
		{
			if (*lst != *vct)
			{
				flag = 0;
...
void PlayScene::ConstructUI() {
	...
	ConstructButton(2, "play/orb-turret.png", OrbTurret::Price);
...
void PlayScene::ConstructButton(int id, std::string sprite, int price) {
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/tower-base.png", 1294 + (id%3) * 76, (136 * (1 + (id/3 * 0.5))), 0, 0, 0, 0),
		Engine::Sprite(sprite, 1294 + (id%3) * 76, (136 * (1 + (id / 3 * 0.5))), 0, 0, 0, 0)
		, 1294 + (id%3) * 76, (136 * (1 + (id / 3 * 0.5))), price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	if (id == 0 && money >= PlugGunTurret::Price)
		...
		preview = new MachineGunTurret(0, 0);
	else if (id == 2 && money >= OrbTurret::Price)
		preview = new OrbTurret(0, 0);
